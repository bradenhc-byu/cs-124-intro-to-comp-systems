...
static uint8 lcd_dmode;				// lcd mode
static uint8 saved_lcd_dmode;
		...
		for (x1 = width; x1 > 0; x1 -= 3)			// display from right to left
		{
			switch (flag)						// switch on mode
			{
				...
				case 1:
					...
					{
						--runCnt;
						WriteDataWord(runPixels);		// output 3 run pixels
						break;
					}
...
void lcd_triangle(int16 x0, int16 y0, uint16 r0, uint8 pen)
{
	...
	do
	{
		lcd_point(x0 - r0/2, y, pen);	// draw end points
		lcd_point(x0 + r0/2, y, pen);
		if (fill)						// if bottom or fill
		{
			for (x = (x0 - r0/2 + 1); x <= (x0 + r0/2 - 1); ++x)
			{
				lcd_point(x, y, pen);
			}
		}
...
unsigned char lcd_putchar(unsigned char c)
{
	...
	if (c < ' ')
	{
		switch (c)
		{
			...
			case '\n':
			{
				lcd_y -= CHAR_SIZE * (lcd_dmode & ~LCD_2X_FONT ? 2 : 1);
				lcd_y %= HD_Y_MAX;
			}
...
uint8 lcd_printf(const char* fmt, ...)
{
	...
	saved_lcd_dmode = lcd_dmode;			// save font mode
	va_list arg_ptr;
...
uint8 lcd_printf(const char* fmt, ...)
{
	...
	while (c = *s_ptr++)
	{
		lcd_putchar(c);						// output string
	}
	va_end(arg_ptr);						// destroy arg pointer
	lcd_dmode = saved_lcd_dmode;			// restore font mode
	return (s_ptr - printBuffer - 1);
} // end lcd_printf
#endif
